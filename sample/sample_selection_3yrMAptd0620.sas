...
* PURPOSE: Selecting Sample - No Part D Requirement
					...
					- Require maptd, enr AB in all 12 months in year t-2 and t-1
					- Require maptd, enr AB all year enrollment in year t 
						(does not need to be 12 months and allows for death);

* INPUT: bene_status_yearYYYY, bene_demog2019;
* OUTPUT: samp_3yrmaptd_0316;
...
%do year=&mindatayear %to &maxdatayear;
	proc freq data=&datalib..bene_status_year&year.;
		table enrHMO_4_mo_yr;
	run;

	data &tempwork..bene&year;
		set &datalib..bene_status_year&year (keep=bene_id age_beg enrHMO_allyr enrHMO_mo_yr 
		enrHMO_4_mo_yr ptD_allyr);
		rename age_beg=age_beg&year enrHMO_allyr=enrHMO_allyr&year enrHMO_mo_yr=enrHMO_mo_yr&year 
		enrHMO_4_mo_yr=enrHMO_4_mo_yr&year. ptD_allyr=ptD_allyr&year.;
	run;
%end;
...
%macro sample;
data &outlib..samp_3yrmaptd_0620;
	...
	* race_drop=(race_bg in("","0","3"));    
	...
	%do year=&minsampyear %to &maxsampyear;
		...
		* Limiting to age 67 and in maptd and Part D in 2 previous years;
		%if &year<=2007 %then %do;
			if age_beg&year>=67 
			and dropflag="N"
			and enrHMO_allyr&year="Y"
			and enrHMO_4_mo_yr&year.=0
		...
		%if &year=2008 %then %do;
			if age_beg&year>=67
			and dropflag="N"
			and (enrHMO_allyr&prev1_year="Y" and enrHMO_allyr&year="Y")
			and (ptD_allyr&prev1_year="Y" and ptD_allyr&year="Y")
			and (enrHMO_mo_yr&prev1_year=12) 
			and (enrHMO_4_mo_yr&prev1_year=0)
			then insamp&year=1;
			else insamp&year=0;
		%end;
		...
		%if &year>=2009 %then %do;
			if age_beg&year>=67 
			and dropflag="N"
			and (enrHMO_allyr&prev2_year="Y" and enrHMO_allyr&prev1_year="Y" and enrHMO_allyr&year="Y")
			and (ptD_allyr&prev2_year="Y" and ptD_allyr&prev1_year="Y" and ptD_allyr&year="Y")
			and (enrHMO_mo_yr&prev2_year=12 and enrHMO_mo_yr&prev1_year=12) 
			and (enrHMO_4_mo_yr&prev2_year=0 and enrHMO_4_mo_yr&prev1_year=0)
			then insamp&year=1;
			else insamp&year=0;
		%end;
...
* By year;
%do year=&minsampyear %to &maxsampyear;
proc freq data=&outlib..samp_3yrmaptd_0620 noprint;
	where insamp&year=1;
...
proc means data=&outlib..samp_3yrmaptd_0620 noprint;
	where insamp&year=1;
...
data &tempwork..allages;
	set
	%do year=&minsampyear %to &maxsampyear;
		&outlib..samp_3yrmaptd_0620 (where=(insamp&year=1) keep=insamp&year bene_id age_beg&year rename=(age_beg&year=age_beg))
	%end;;
run;
...
proc export data=samp_stats_maptd
	outfile="&rootpath./Projects/Programs/base/exports/samp_stats_maptd0620.xlsx"
	dbms=xlsx
	replace;
	sheet="stats";
run;

proc contents data=&outlib..samp_3yrmaptd_0620; run;
