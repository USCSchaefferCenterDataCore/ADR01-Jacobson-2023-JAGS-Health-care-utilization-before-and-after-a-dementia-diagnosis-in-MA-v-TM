...
* Author: PF;
* Purpose: Building claims on date-level;
* Input: Pulled dementia claims; 
* Output: dementia_dxdt_ma15_16;
...
%macro combine_dts(typ,dropv=);
		...
		title2 dementia_dx_&typ._ma&minyear._&maxyear to dementia_dt_&typ._ma&minyear._&maxyear;
		proc sort data=&outlib..dementia_dx_&typ._adj_ma&minyear._&maxyear.; by bene_id clm_thru_dt; run;

		data &tempwork..dementia_dt_&typ._ma&minyear._&maxyear.;
				set &outlib..dementia_dx_&typ._adj_ma&minyear._&maxyear (drop=&dropv where=(not missing(bene_id)));
				...
				* First claim on this date. Save dementia dx-s into master list;
				if first.clm_thru_dt=1 then do;
						do i=1 to dim(demdx);
							_demdx[i]=demdx[i];
							* resetting dxtypes because the previous ones could have codes previously deleted;
      	...
      	* output one obs per service_type and date;
      	if last.clm_thru_dt=1 then do;
            ...
            ;
        keep bene_id clm_thru_dt demdx: clm_typ n_add_typ n_claim_typ dxtypes bene: dx_max;
...
***** Took out the part of the code that keeps physicians; 

*****  Merge all types together;
data &outlib..dementia_dt_ma&minyear._&maxyear.;
		
		merge &tempwork..dementia_dt_ip_ma&minyear._&maxyear (drop=dxtypes) 
			  &tempwork..dementia_dt_op_ma&minyear._&maxyear (drop=dxtypes)
			  &tempwork..dementia_dt_snf_ma&minyear._&maxyear(drop=dxtypes)
			  &tempwork..dementia_dt_hha_ma&minyear._&maxyear(drop=dxtypes)
			  &tempwork..dementia_dt_carrier_ma&minyear._&maxyear (drop=dxtypes);
		by bene_id clm_thru_dt clm_typ;
				
		mult_clmtyp=(first.clm_typ=0 or last.clm_typ=0);
		
		* Output summarized events, across claim types on same date?;
		length claim_types $ 5 dxtypes $ 13 _demdx1-_demdx&max_demdx $ 5;
		...
		retain claim_types n_claims _dxmax _dxmax1 _demdx1-_demdx&max_demdx;
    ...
    label n_add_dxdate="# of dementia dx added from additional claim types"
         ...
         ;
   	drop _demdx: _dxmax _dxmax1 i j dxfound clm_typ n_claim_typ n_add_typ;

run;

proc contents data=&outlib..dementia_dt_ma&minyear._&maxyear; run;
