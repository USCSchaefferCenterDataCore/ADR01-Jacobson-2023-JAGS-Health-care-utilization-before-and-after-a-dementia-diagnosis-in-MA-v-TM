...
* Input: plwd_mortnonsnp, arthglaunonsnp, mortalitydddnonsnp, maffs.ipincdx_bene;
...
/* DD */
data plwd_mortnonsnp_loc;
	merge plwd_mortnonsnp (in=a) maffs.ipincdx_bene (in=b keep=bene_id ipincdx);
	by bene_id;
	if a;
run;
...
proc logistic data=&cond._mortnonsnp_loc;
...
/* DDD */
proc sort data=mortalitydddnonsnp; by bene_id; run;

data mortalitydddnonsnp_loc;
	merge mortalitydddnonsnp (in=a) &outlib..ipincdx_bene (in=b keep=bene_id ipincdx);
	by bene_id;
	if a;
run;
...
proc logistic data=mortalitydddnonsnp_loc;
